...
#include <spudlog/details/file_helper.h>
...
template <typename Mutex, class Alloc>
SPDLOG_INLINE void rotating_file_sink<Mutex, Alloc>::rotate_now() {
    std::lock_guard<Mutex> lock(base_sink<Mutex, Alloc>::mutex_);
    rotate_();
}
template <typename Mutex, class Alloc>
SPDLOG_INLINE void rotating_file_sink<Mutex, Alloc>::set_max_size(std::size_t max_size) {
    std::lock_guard<Mutex> lock(base_sink<Mutex, Alloc>::mutex_);
    if (max_size == 0) {
...
template <typename Mutex, class Alloc>
SPDLOG_INLINE std::size_t rotating_file_sink<Mutex, Alloc>::get_max_size() {
    std::lock_guard<Mutex> lock(base_sink<Mutex, Alloc>::mutex_);
    return max_size_;
}

template <typename Mutex, class Alloc>
SPDLOG_INLINE void rotating_file_sink<Mutex, Alloc>::set_max_files(std::size_t max_files) {
    std::lock_guard<Mutex> lock(base_sink<Mutex, Alloc>::mutex_);
    if (max_files > MaxFiles) {
...
template <typename Mutex, class Alloc>
std::size_t rotating_file_sink<Mutex, Alloc>::get_max_files() {
    std::lock_guard<Mutex> lock(base_sink<Mutex, Alloc>::mutex_);
    return max_files_;
}
