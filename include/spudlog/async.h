...
struct async_factory_impl {
    ...
    static std::shared_ptr<basic_async_logger<Alloc>> create(std::string logger_name,
                                                             SinkArgs &&...args) {
        auto &registry_inst = details::registry<Alloc>::instance();
        ...
        if (tp == nullptr) {
            tp = std::make_shared<details::basic_thread_pool<Alloc>>(details::default_async_q_size, 1U);
            registry_inst.set_tp(tp);
        }

        auto sink = std::make_shared<Sink>(std::forward<SinkArgs>(args)...);
        auto new_logger = std::make_shared<basic_async_logger<Alloc>>(
